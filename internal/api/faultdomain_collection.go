...
// FaultDomainCollectionHandler serves FaultDomain LIST and create.
type FaultDomainCollectionHandler struct {
...
// NewFaultDomainCollectionHandler constructs the collection handler.
func NewFaultDomainCollectionHandler(
	store *cloudmodel.Store,
	grants GrantResolver,
	pub *cloudmodel.PublicationCoordinator,
	idemp *cloudmodel.IdempotencyCoordinator,
) *FaultDomainCollectionHandler {
	return &FaultDomainCollectionHandler{
		Store: store, Grants: grants, Publication: pub, Idempotency: idemp,
	}
}

// ServeHTTP dispatches GET (LIST) and POST (create).
func (h *FaultDomainCollectionHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		h.list(w, r)
...
func (h *FaultDomainCollectionHandler) list(w http.ResponseWriter, r *http.Request) {
	...
	items := h.Store.ListFaultDomains()
	visible := make([]model.FaultDomain, 0, len(items))
	for _, item := range items {
		if topologyReadable(h.Grants, item.Metadata.ScopeRef, item.Metadata.UID) {
			visible = append(visible, item)
		}
	}
	writeJSONSuccess(w, r, http.StatusOK, apimeta.ListEnvelope[model.FaultDomain]{
		TypeMeta: apimeta.TypeMeta{APIVersion: model.APIVersionFaultDomain, Kind: model.KindFaultDomain + "List"},
		Items:    visible,
	})
}

func (h *FaultDomainCollectionHandler) create(w http.ResponseWriter, r *http.Request) {
	principal, prob := authenticatePrincipal(r, h.Grants)
	if prob != nil {
		writeProblem(w, r, prob)
		return
	...
	parentRef, hasRef := phaseOne.Refs["spec.datacenterRef"]
	var (
	...
	if hasRef && parentRef.UID != "" {
		var parentOK bool
		parent, parentOK = h.Store.GetDatacenter(parentRef.UID)
		var visible bool
	...
	var decoded model.FaultDomain
	if prob := DecodePhaseTwoStrict(validate.KindFaultDomain, validate.RequestClassCollectionCreate, phaseOne.Body, &decoded); prob != nil {
	...
	if scopeRef == nil {
		writeProblem(w, r, apiproblem.New(apiproblem.CodeValidationFailed).WithViolations([]apiproblem.Violation{{
			...
			Message: "datacenterRef uid is required",
		}}))
		return
	}

	digest, err := cloudmodel.CanonicalDigest(cloudmodel.CanonicalDigestInput{
		Body: phaseOne.Body, Pattern: PatternFaultDomainCreate,
	})
	if err != nil {
	...
	ns := cloudmodel.IdempotencyNamespace{
		PrincipalID: principal, Pattern: PatternFaultDomainCreate, Key: r.Header.Get("Idempotency-Key"),
	}
	...
	decoded.TypeMeta = apimeta.TypeMeta{APIVersion: model.APIVersionFaultDomain, Kind: model.KindFaultDomain}
	...
	decoded.Status = model.FaultDomainStatus{Phase: model.FaultDomainPhaseActive}

	if prob := runFeature0015StageSet(r.Context(), validate.KindFaultDomain, validate.RequestClassCollectionCreate, decoded); prob != nil {
		h.Idempotency.Abort(ns)
		writeProblem(w, r, prob)
		return
	}
	if prob := validate.ValidateFaultDomainParentChain(decoded, parent); prob != nil {
		h.Idempotency.Abort(ns)
		writeProblem(w, r, prob)
		return
	}
	...
	staged, stageProb := h.Store.StageCreateFaultDomain(decoded)
	if stageProb != nil {
		h.Idempotency.Abort(ns)
	...
	event := cloudmodel.NewRedactedAuditEvent(cloudmodel.RedactedAuditInput{
		...
		Reason: "faultdomain.created", Actor: actorRef(principal),
