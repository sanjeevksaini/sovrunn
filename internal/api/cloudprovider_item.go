...
// CloudProviderItemHandler serves CloudProvider GET and PATCH.
type CloudProviderItemHandler struct {
	Store       *cloudmodel.Store
	Grants      GrantResolver
	Publication *cloudmodel.PublicationCoordinator
}

// NewCloudProviderItemHandler constructs the item handler.
func NewCloudProviderItemHandler(
	store *cloudmodel.Store,
	grants GrantResolver,
	pub *cloudmodel.PublicationCoordinator,
) *CloudProviderItemHandler {
	return &CloudProviderItemHandler{Store: store, Grants: grants, Publication: pub}
}

// ServeHTTP dispatches GET and PATCH. PUT/DELETE/HEAD → 405.
func (h *CloudProviderItemHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	uid := r.PathValue("uid")
	switch r.Method {
	case http.MethodGet:
		h.get(w, r, uid)
...
func (h *CloudProviderItemHandler) get(w http.ResponseWriter, r *http.Request, uid string) {
	...
	if uid == "" || !h.Grants.AuthorizeExact(cloudmodel.ActionCloudProviderRead, cloudmodel.PlatformRootScope, uid, "") {
		writeAuditedDenial(w, r, h.Publication, safeDenialGET(principal, requestID(r)))
		return
	}
	cp, ok := h.Store.GetCloudProvider(uid)
	if !ok {
		writeAuditedDenial(w, r, h.Publication, safeDenialGET(principal, requestID(r)))
		return
	}
...
func (h *CloudProviderItemHandler) patch(w http.ResponseWriter, r *http.Request, uid string) {
	principal, prob := authenticatePrincipal(r, h.Grants)
	if prob != nil {
		writeProblem(w, r, prob)
		return
	...
	if len(h.Grants.CoarseLookup(cloudmodel.ActionCloudProviderWrite)) == 0 ||
		!h.Grants.AuthorizeExact(cloudmodel.ActionCloudProviderWrite, cloudmodel.PlatformRootScope, uid, "") {
		writeProblem(w, r, apiproblem.New(apiproblem.CodeAuthorizationDenied).WithDetail("cloud-provider-admin write grant is required"))
		return
	}

	current, ok := h.Store.GetCloudProvider(uid)
	if !ok {
		writeAuditedDenial(w, r, h.Publication, safeDenialGET(principal, requestID(r)))
		return
	}

	if prob := DecodePhaseTwoStrict(validate.KindCloudProvider, validate.RequestClassPatch, phaseOne.Body, nil); prob != nil {
		writeClassifiedCreateDenial(w, r, h.Publication, principal, prob)
		return
	}
	mergedAny, prob := ApplyMergePatch(validate.KindCloudProvider, current, phaseOne.Body)
	if prob != nil {
		writeClassifiedCreateDenial(w, r, h.Publication, principal, prob)
		return
	}
	merged, ok := mergedAny.(model.CloudProvider)
	if !ok {
		writeProblem(w, r, apiproblem.New(apiproblem.CodeInternalError).WithDetail("merge produced unexpected type"))
		return
	}
	if prob := runFeature0015StageSet(r.Context(), validate.KindCloudProvider, validate.RequestClassPatch, merged); prob != nil {
		writeProblem(w, r, prob)
		return
	}
	...
	live, ok := h.Store.LookupCloudProvider(uid)
	if !ok {
		writeAuditedDenial(w, r, h.Publication, safeDenialGET(principal, requestID(r)))
		return
	}
	...
	staged, stageProb := h.Store.StageUpdateCloudProvider(merged)
	if stageProb != nil {
	...
	event := cloudmodel.NewRedactedAuditEvent(cloudmodel.RedactedAuditInput{
		...
		Reason: "cloudprovider.patched", Actor: actorRef(principal),
	...
	updated, _ := h.Store.LookupCloudProvider(uid)
