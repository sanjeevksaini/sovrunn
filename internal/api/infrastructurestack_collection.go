...
// InfrastructureStackCollectionHandler serves InfrastructureStack LIST and create.
type InfrastructureStackCollectionHandler struct {
...
// NewInfrastructureStackCollectionHandler constructs the collection handler.
func NewInfrastructureStackCollectionHandler(
	store *cloudmodel.Store,
	grants GrantResolver,
	pub *cloudmodel.PublicationCoordinator,
	idemp *cloudmodel.IdempotencyCoordinator,
) *InfrastructureStackCollectionHandler {
	return &InfrastructureStackCollectionHandler{
		Store: store, Grants: grants, Publication: pub, Idempotency: idemp,
	}
}

// ServeHTTP dispatches GET (LIST) and POST (create).
func (h *InfrastructureStackCollectionHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		h.list(w, r)
...
func (h *InfrastructureStackCollectionHandler) list(w http.ResponseWriter, r *http.Request) {
	...
	items := h.Store.ListInfrastructureStacks()
	visible := make([]model.InfrastructureStack, 0, len(items))
	for _, item := range items {
		if topologyReadable(h.Grants, item.Metadata.ScopeRef, item.Metadata.UID) {
			visible = append(visible, item)
		}
	}
	writeJSONSuccess(w, r, http.StatusOK, apimeta.ListEnvelope[model.InfrastructureStack]{
		TypeMeta: apimeta.TypeMeta{APIVersion: model.APIVersionInfrastructureStack, Kind: model.KindInfrastructureStack + "List"},
		Items:    visible,
	})
}

func (h *InfrastructureStackCollectionHandler) create(w http.ResponseWriter, r *http.Request) {
	principal, prob := authenticatePrincipal(r, h.Grants)
	if prob != nil {
		writeProblem(w, r, prob)
		return
	...
	parentRef, hasRef := phaseOne.Refs["spec.faultDomainRef"]
	var (
		parent   model.FaultDomain
		scopeRef *apimeta.ScopeRef
	)
	if hasRef && parentRef.UID != "" {
		var parentOK bool
		parent, parentOK = h.Store.GetFaultDomain(parentRef.UID)
		var visible bool
	...
	var decoded model.InfrastructureStack
	if prob := DecodePhaseTwoStrict(validate.KindInfrastructureStack, validate.RequestClassCollectionCreate, phaseOne.Body, &decoded); prob != nil {
	...
	if scopeRef == nil {
		writeProblem(w, r, apiproblem.New(apiproblem.CodeValidationFailed).WithViolations([]apiproblem.Violation{{
			...
			Message: "faultDomainRef uid is required",
		}}))
		return
	}

	digest, err := cloudmodel.CanonicalDigest(cloudmodel.CanonicalDigestInput{
		Body: phaseOne.Body, Pattern: PatternInfrastructureStackCreate,
	})
	if err != nil {
	...
	ns := cloudmodel.IdempotencyNamespace{
		PrincipalID: principal, Pattern: PatternInfrastructureStackCreate, Key: r.Header.Get("Idempotency-Key"),
	}
	...
	decoded.TypeMeta = apimeta.TypeMeta{APIVersion: model.APIVersionInfrastructureStack, Kind: model.KindInfrastructureStack}
	...
	decoded.Status = model.InfrastructureStackStatus{Phase: model.InfrastructureStackPhaseActive}

	if prob := runFeature0015StageSet(r.Context(), validate.KindInfrastructureStack, validate.RequestClassCollectionCreate, decoded); prob != nil {
		h.Idempotency.Abort(ns)
		writeProblem(w, r, prob)
		return
	}
	if prob := validate.ValidateInfrastructureStackParentChain(decoded, parent); prob != nil {
		h.Idempotency.Abort(ns)
		writeProblem(w, r, prob)
		return
	}
	...
	staged, stageProb := h.Store.StageCreateInfrastructureStack(decoded)
	if stageProb != nil {
		h.Idempotency.Abort(ns)
	...
	event := cloudmodel.NewRedactedAuditEvent(cloudmodel.RedactedAuditInput{
		...
		Reason: "infrastructurestack.created", Actor: actorRef(principal),
