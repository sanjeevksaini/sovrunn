...
// HostingLocationItemHandler serves HostingLocation GET and PATCH.
type HostingLocationItemHandler struct {
	Store       *cloudmodel.Store
	Grants      GrantResolver
	Publication *cloudmodel.PublicationCoordinator
}

// NewHostingLocationItemHandler constructs the item handler.
func NewHostingLocationItemHandler(
	store *cloudmodel.Store,
	grants GrantResolver,
	pub *cloudmodel.PublicationCoordinator,
) *HostingLocationItemHandler {
	return &HostingLocationItemHandler{Store: store, Grants: grants, Publication: pub}
}

// ServeHTTP dispatches GET and PATCH. PUT/DELETE/HEAD → 405.
func (h *HostingLocationItemHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	uid := r.PathValue("uid")
	switch r.Method {
	case http.MethodGet:
		h.get(w, r, uid)
...
func (h *HostingLocationItemHandler) get(w http.ResponseWriter, r *http.Request, uid string) {
	...
	hl, ok := h.Store.GetHostingLocation(uid)
	if uid == "" || !ok || !topologyReadable(h.Grants, hl.Metadata.ScopeRef, uid) {
		writeAuditedDenial(w, r, h.Publication, safeDenialGET(principal, requestID(r)))
		return
	}
	writeJSONSuccess(w, r, http.StatusOK, hl)
}

func (h *HostingLocationItemHandler) patch(w http.ResponseWriter, r *http.Request, uid string) {
	principal, prob := authenticatePrincipal(r, h.Grants)
	if prob != nil {
		writeProblem(w, r, prob)
		return
	...
	current, ok := h.Store.GetHostingLocation(uid)
	...
	if prob := DecodePhaseTwoStrict(validate.KindHostingLocation, validate.RequestClassPatch, phaseOne.Body, nil); prob != nil {
		writeClassifiedCreateDenial(w, r, h.Publication, principal, prob)
		return
	}
	mergedAny, prob := ApplyMergePatch(validate.KindHostingLocation, current, phaseOne.Body)
	if prob != nil {
		writeClassifiedCreateDenial(w, r, h.Publication, principal, prob)
		return
	}
	merged, ok := mergedAny.(model.HostingLocation)
	if !ok {
		writeProblem(w, r, apiproblem.New(apiproblem.CodeInternalError).WithDetail("merge produced unexpected type"))
		return
	}
	if prob := runFeature0015StageSet(r.Context(), validate.KindHostingLocation, validate.RequestClassPatch, merged); prob != nil {
		writeProblem(w, r, prob)
		return
	}
	...
	live, ok := h.Store.LookupHostingLocation(uid)
	if !ok {
		writeAuditedDenial(w, r, h.Publication, safeDenialGET(principal, requestID(r)))
		return
	}
	...
	staged, stageProb := h.Store.StageUpdateHostingLocation(merged)
	if stageProb != nil {
	...
	event := cloudmodel.NewRedactedAuditEvent(cloudmodel.RedactedAuditInput{
		...
		Reason: "hostinglocation.patched", Actor: actorRef(principal),
	...
	updated, _ := h.Store.LookupHostingLocation(uid)
