...
`ADH-2026-047` (approved 2026-08-12) corrects this handoff's topology-name-mutability wording, without changing this handoff's approved product decision:

3. **Topology name immutability.** The "Topology" row of the Canonical resources, scope, and update surface table below states "Name and description are PATCHable." That sentence is corrected to: **"Description is PATCHable; name is immutable identity."** `metadata.name` is immutable identity for every FEATURE-0015 resource, including topology resources (`HostingLocation`, `Datacenter`, `FaultDomain`, `InfrastructureStack`); only `spec.description` is PATCHable for those resources. This is a wording correction only — the registry's topology mutability fields already stated identity/scope immutability and PATCH-only `spec.description` and required no field-level change.

See `docs/reviews/architecture-decision-handoffs/ADH-2026-047-feature-0015-request-construction-and-scope-derivation-closure.md` for the complete correction.
