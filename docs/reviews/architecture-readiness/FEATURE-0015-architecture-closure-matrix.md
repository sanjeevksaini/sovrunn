...
- Prepared: 2026-08-10
- Status: **Proposed — architecture-owner decision required before requirements approval**
...
Requirements generation and requirements review remain paused. The existing generated requirements document is evidence only and must not be approved for design until every `OPEN` row below is closed and architecture-owner approval is recorded.
...
- [ ] Each ARC-F15 row is `RESOLVED`, with the approving ADH/DEC/ACR reference.
- [ ] Architecture-owner approval is recorded against the complete impacted-document package.
- [ ] Registry, feature authority, architecture boundary, contract specification, and traceability agree.
- [ ] A feature-generic architecture-readiness check validates the seven categories represented by ARC-F15-01 through ARC-F15-07 before Kiro requirements generation can run.
