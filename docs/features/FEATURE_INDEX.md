...
| FEATURE-0013 | Decision Record and AuditEvent Standard | Phase 2: Reuse-First PaaS Fabric Foundation | Implemented and Merged | `decision-object-and-auditevent-standard` | Implements the approved ADH-2026-017 DecisionRecord and AuditEvent contract through PR #15, merged 2026-07-29 into `phase2-reuse-first-paas-fabric-foundation`; preserves FEATURE-0012's six governance scopes and models ServiceInstance as a typed subject within its governing scope. |
| FEATURE-0014 | Provider-Neutral Resource Model | Phase 2: Reuse-First PaaS Fabric Foundation | Implemented and Merged | `provider-neutral-resource-model` | Implements Provider, ProviderLocation, ProviderDatacenter, DatacenterFailureDomain, and InfrastructureStack under ADH-2026-018 as clarified by ADH-2026-019; final gate passed and PR #16 merged 2026-07-30 as commit `1ed47ac` into `phase2-reuse-first-paas-fabric-foundation`. |
| FEATURE-0015 | Canonical Cloud Model Foundation | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `canonical-cloud-model-and-alpha-migration` | Seven-scope canonical bootstrap; CloudPlatform; CloudProvider; CloudProviderParticipation; HostingLocation; Datacenter; FaultDomain; InfrastructureStack (ends here); no ResourcePool/ProviderCapability; no alpha runtime migration (DEC-0059). |
| FEATURE-0016 | Adapter Boundary and ExecutionTarget Qualification | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `adapter-boundary-and-executiontarget-qualification` | Core adapter interfaces; normalized target facts; target qualification and availability axes; target-scoped SecretRefs; fake adapter. |
| FEATURE-0017 | Policy Evaluation Abstraction | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `policy-evaluation-abstraction` | PolicyEvaluationRequest/Result; PolicyEngineAdapter; DecisionRecord linkage; deterministic bootstrap fake only. |
| FEATURE-0018 | Governance, IAM, Approval and Exception Foundation | Phase 2R: Canonical Model PaaS Fabric Foundation | Implemented and Merged | `governance-iam-approval-exception-foundation` | AccessGroup; Membership; RoleDefinition; RoleAssignment; exact-Human PrincipalRef-only approval/review/privileged eligibility; PrivilegedAccessRequest; AccessReview; ApprovalPolicy; ApprovalRequest; ExceptionGrant; FEATURE-0018-limited GovernanceProfile. Canonical architecture: `docs/architecture/FEATURE-0018-governance-iam-approval-exception-foundation.md`; conformance correction: ADH-2026-071; PR #20 merged 2026-09-07 as commit `2295e9e` into `phase2-reuse-first-paas-fabric-foundation`. |
| FEATURE-0019 | Sovereignty Facts, Evidence and Policy Foundation | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `sovereignty-facts-evidence-policy-foundation` | SovereigntyProfile; RegulatoryPolicyBundle; SovereigntyFactSet; EvidenceRecord; governed dimension registry. |
| FEATURE-0020 | Assignment and Effective Governance Resolution | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `assignment-and-effective-governance-resolution` | ProfileAssignment; immutable EffectiveGovernanceContext; deterministic inheritance/conflict/exception resolution. |
| FEATURE-0021 | CloudEnrollment, Personal Onboarding, Entitlement and Quota | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `cloudenrollment-onboarding-entitlement-quota` | CloudEnrollment; Personal Organization; EntitlementPackage; ServiceEntitlement; QuotaPolicy; provider-selection intent. |
| FEATURE-0022 | Service Product, Runtime and Requirement Foundation | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `service-product-runtime-requirement-foundation` | ServicePortfolio; ServiceTypeDefinition; ServiceOffering; ServicePlan migration; ServicePlacementProfile; ServiceRuntimeProfile; ServiceRequirementSet. |
| FEATURE-0023 | Sovereignty and Placement Decision v0 | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `sovereignty-and-placement-decision-v0` | Registered DecisionProfiles; authoritative DecisionRecords; candidate-set evaluation; ServicePlacement safe projection. |
| FEATURE-0024 | Plugin Taxonomy and Synthetic Execution Boundary | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `plugin-taxonomy-and-synthetic-execution-boundary` | Plugin/adapter roles; manifests; compatibility; protected handles; PluginExecution contract; deterministic fake harness. |
| FEATURE-0025 | AI-Readable Decision and Operation Context | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `ai-readable-decision-and-operation-context` | Bounded audience-safe explanation projection from decisions, evidence, operations, and audit. |
| FEATURE-0026 | Slice 0 Integration and Conformance Demo | Phase 2R: Canonical Model PaaS Fabric Foundation | Executable | `slice-0-integration-and-conformance-demo` | Cross-feature orchestration of one synthetic service through fake execution, safe placement, and fake binding; VS-000 conformance. |
| FEATURE-0027 | Plugin Execution Contract v0 | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `plugin-execution-contract-v0` | Production-shaped PluginExecution attempt/transport contract. |
| FEATURE-0028 | Operation Controller v0 | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `operation-controller-v0` | Operation orchestration controller with step tracking and approval linkage. |
| FEATURE-0029 | PostgreSQL Management Plane Plugin v0 | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `postgresql-management-plane-plugin-v0` | PostgreSQL management plugin using mature operator/Helm wrappers. |
| FEATURE-0030 | Kubernetes / Local Substrate Plugin v0 | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `kubernetes-local-substrate-plugin-v0` | OpenShift/Kubernetes execution adapter for one substrate. |
| FEATURE-0031 | PostgreSQL Runtime Plugin v0 | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `postgresql-runtime-plugin-v0` | Mature PostgreSQL operator integration for runtime lifecycle. |
| FEATURE-0032 | ServiceInstance Provisioning v0 | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `serviceinstance-provisioning-v0` | Service realization pipeline from approved plan to running instance. |
| FEATURE-0033 | ServiceBinding and SecretRef Integration | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `servicebinding-and-secretref-integration` | Workload identity and SecretRef binding for consumers. |
| FEATURE-0034 | Phase 3 End-to-End MVP Demo | Phase 3: First Executable PaaS Plugin Chain | MVP Planning | `phase-3-end-to-end-mvp-demo` | PostgreSQL end-to-end conformance demo on one substrate. |
...
| FEATURE-0047 | Provider Onboarding Workflow | Phase 5: Provider / Plugin Framework and Certification | Roadmap Placeholder | `provider-onboarding-workflow` | Guide CloudProvider/MSP through CloudPlatform participation, HostingLocation, InfrastructureStack, ExecutionTarget qualification, and plugin onboarding. Terminology aligns with canonical model after Phase 2R revalidation. |
...
| FEATURE-0061 | Capacity Model (future) | Phase 7: Autoscaling, Capacity, Cost, and Spot | Roadmap Placeholder | `capacity-model-future` | Track capacity availability, reservation, and exhaustion signals. Note: ResourcePool is not a canonical concept (DEC-0042); this feature will be revalidated after Phase 2R using ExecutionTarget-based capacity semantics. |
...
| FEATURE-0096 | SDE ServiceTypeDefinition and ServiceOffering | Phase 11: SDE as Managed Service | Roadmap Placeholder | `sde-service-type-and-offering` | Model SDE as a governed managed service using ServiceTypeDefinition + ServiceOffering (DEC-0049). ServiceClass is migration input only. |
