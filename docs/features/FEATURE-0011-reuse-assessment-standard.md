...
|---|---|---|---|
| Incorrect capability classification | Capability-level disposition rules and closed vocabularies in the canonical standard | RA-S03 structural validation; human semantic review | Correct assessment disposition field |
| Vendor-first architecture | Provider-neutrality field; no vendor selection in Phase 2 | Human review of candidates and non-goals | Remove vendor coupling; re-assess |
| Direct external coupling | Mandatory adapter fields and DEC-0036 traceability for adapter-related assessments | RA-C01/RA-C02 checks; human review | Add adapter boundary decision |
| Wrapper responsibility expansion | Wrap semantics that prohibit recreating wrapped engines | Human review of boundary fields | Narrow wrapper scope definition |
| Unjustified custom build | Build triple rejection-rationale requirement | RA-C07 check; human review | Provide Build rationale or re-disposition |
| Adapter omission | Mandatory adapter fields and rationale | RA-C01/RA-C06 checks | Add adapter decision/identifier |
| Phase-scope leakage | Current-phase Yes/No, deferred work, and non-goals fields | RA-C03 check; phase2-scope-check.sh | Move content to deferred/non-goals |
| Missing replacement planning | Exit/migration boundary and replacement-risk fields | RA-S08 structural checks on replacement risk | Add exit boundary and replacement risk |
| Architecture-status ambiguity | Single canonical decision-status field | RA-S04 decision-status check | Set exact decision status |
| Template divergence | Align-by-reference rule for prompts, templates, and factory documents | RA-C12; RA-C14 | Re-align to canonical version |
| Traceability errors | Mandatory DEC/RFC/ADH traceability fields | RA-C05 reference-existence check | Correct references |
| Silent later-feature overrides | Change-control requiring a new ADH for post-approval disposition or boundary changes | Human review of new ADH and prior assessment traceability | Require ADH; restore Approved assessment |
...
| Structured approval-evidence record | `docs/reviews/reuse-assessments/FEATURE-0011-approval-evidence.md` |
| Scope of approval | Approves the Extend disposition and the Sovrunn-owned versus reused/extended responsibility boundary recorded in this assessment, as stated in ADH-2026-011 Human approval and the structured approval-evidence record |

This assessment decision status is **Approved** for the governance-contract
capability through ADH-2026-011. Deterministic RA-C13 approval evidence is
the structured record at
`docs/reviews/reuse-assessments/FEATURE-0011-approval-evidence.md`. That
assessment approval is distinct from final FEATURE-0011 merge review, which
remains a separate human action and must not be inferred from this document.
