...
Document:
  ID: mysql-protocol-specification
  Title: MySQL Protocol Specification
...
Purpose:
  - Define the MySQL protocol integration contract for Sovrunn Data Engine
  - Define Protocol Plugin responsibilities for MySQL
...
Definition:
  MySQL Protocol Specification defines how SDE integrates MySQL-compatible clients through a MySQL Protocol Plugin while preserving MySQL client-visible request, session, transaction, result, and error behavior.
...
Protocol Identity:
  Protocol: MySQL
  Canonical Identifier: sde.protocol.mysql
...
Protocol Plugin Responsibilities:
  MUST:
    ...
    - Map Result Model to MySQL-compatible response
    - Map Error Model to MySQL-compatible error response
...
Supported Request Forms:
  ...
  - Result-set metadata exchange
  - Command execution
  - Session variable handling

Request Parsing Rules:
  - Protocol Plugin MUST parse protocol input according to MySQL rules.
...
Session Rules:
  - MySQL session variables MUST be preserved where supported.
  - Connection capability flags MUST be interpreted safely.
  - Prepared statement identifiers MUST be session-scoped.
  - Character set and collation semantics MUST be declared.
  - Invalid session state MUST fail closed.

Transaction Rules:
  - START TRANSACTION, COMMIT, and ROLLBACK MUST be recognized.
  - Autocommit semantics MUST be preserved.
  - Isolation level support MUST be explicit.
  - Unsupported transactional behavior MUST fail deterministically.
  - Transaction uncertainty MUST be explicit.
...
Result Mapping Rules:
  ...
  - Result-set column metadata MUST preserve MySQL-visible type behavior.
  - OK packet affected count and status flags MUST be mapped safely.
...
Error Mapping Rules:
  ...
  - MySQL error code and SQLSTATE mapping SHOULD be used where safe and supported.
...
Invariants:
  - MySQL Protocol Plugin owns MySQL-specific parsing and mapping.
