# PostgreSQL Protocol Specification

Document:
  ID: postgresql-protocol-specification
  Title: PostgreSQL Protocol Specification
...
Purpose:
  - Define the PostgreSQL protocol integration contract for Sovrunn Data Engine
  - Define Protocol Plugin responsibilities for PostgreSQL
...
Definition:
  PostgreSQL Protocol Specification defines how SDE integrates PostgreSQL-compatible clients through a PostgreSQL Protocol Plugin while preserving PostgreSQL wire-visible behavior and mapping requests into SDE protocol-normalized intent.
...
Protocol Identity:
  Protocol: PostgreSQL
  Canonical Identifier: sde.protocol.postgresql
  Plugin Type: Protocol Plugin

Protocol Plugin Responsibilities:
  MUST:
    - Parse PostgreSQL input
    ...
    - Map Result Model to PostgreSQL-compatible response
    - Map Error Model to PostgreSQL-compatible error response
...
Supported Request Forms:
  ...
  - Portal and cursor behavior
  - Runtime parameter exchange

Request Parsing Rules:
  - Protocol Plugin MUST parse protocol input according to PostgreSQL rules.
...
Session Rules:
  - PostgreSQL session parameters MUST be preserved where supported.
  - Prepared statement and portal references MUST be session-scoped.
  - Runtime parameter changes MUST be represented safely.
  - Session state MUST be isolated by tenant and connection.
  - Invalid session state MUST fail closed.

Transaction Rules:
  - Explicit BEGIN, COMMIT, and ROLLBACK MUST be recognized.
  - Autocommit behavior MUST preserve PostgreSQL-visible semantics.
  - Savepoint behavior MUST be declared when supported.
  - Isolation level support MUST be explicit.
...
Result Mapping Rules:
  ...
  - Row description and data row behavior MUST preserve PostgreSQL-visible type metadata.
  - Command completion tags MUST preserve affected-count semantics where applicable.
...
Error Mapping Rules:
  ...
  - SQLSTATE-compatible error mapping SHOULD be used where safe and supported.
...
Invariants:
  - PostgreSQL Protocol Plugin owns PostgreSQL-specific parsing and mapping.
