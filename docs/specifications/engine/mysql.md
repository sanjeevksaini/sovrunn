...
Document:
  ID: mysql-engine-specification
  Title: MySQL Engine Specification
...
Purpose:
  - Define the MySQL Downstream Engine integration contract for Sovrunn Data Engine
  - Define Engine Plugin responsibilities for MySQL
  - Define execution fragment translation, downstream invocation, native result mapping, and native error mapping rules
  - Preserve separation between SDE execution semantics and MySQL native behavior

Definition:
  MySQL Engine Specification defines how SDE integrates a MySQL-compatible Downstream Datastore through a MySQL Engine Plugin.
...
Engine Identity:
  Downstream Engine: MySQL
  Canonical Identifier: sde.engine.mysql
...
Engine Plugin Responsibilities:
  MUST:
    ...
    - Translate fragment into MySQL-native operation
    - Preserve semantic equivalence
    - Invoke Downstream Datastore through approved MySQL interface
...
Translation Rules:
  ...
  - MySQL dialect behavior MUST be declared.
  - Character set and collation impact MUST be explicit.
  - Unsupported MySQL feature MUST fail deterministically.
...
Downstream Invocation Rules:
  ...
  - Connection pool behavior MUST preserve tenant and credential boundaries.
  - Autocommit and transaction-bound execution MUST preserve semantics.

Datastore Data Plane Boundary:
  - MySQL Downstream Datastore owns its Datastore Data Plane.
  - SDE Data Plane reaches MySQL Datastore Data Plane only through approved Engine Plugin.
  - Native execution semantics remain owned by MySQL.
  - SDE platform semantics MUST not be silently redefined by MySQL native behavior.
...
Result Mapping Rules:
  ...
  - Column metadata and affected count MUST map safely.
  - Generated keys or insert identifiers MUST be preserved where applicable.
...
Error Mapping Rules:
  ...
  - MySQL error code and SQLSTATE mapping SHOULD be preserved where safe.
...
Invariants:
  - MySQL Engine Plugin is the downstream execution boundary.
  - Engine Runtime invokes MySQL Engine Plugin.
  - MySQL Engine Plugin invokes MySQL Downstream Datastore.
  - MySQL Engine Plugin does not manage datastore lifecycle.
