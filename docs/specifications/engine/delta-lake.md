# Delta Lake Engine Specification

Document:
  ID: delta-lake-engine-specification
  Title: Delta Lake Engine Specification
...
Purpose:
  - Define the Delta Lake Downstream Engine integration contract for Sovrunn Data Engine
  - Define Engine Plugin responsibilities for Delta Lake
  - Define execution fragment translation, downstream invocation, native result mapping, and native error mapping rules
  - Preserve separation between SDE execution semantics and Delta Lake native behavior

Definition:
  Delta Lake Engine Specification defines how SDE integrates Delta Lake table log and data operations through a Delta Lake Engine Plugin.
...
Engine Identity:
  Downstream Engine: Delta Lake
  Canonical Identifier: sde.engine.delta-lake
...
Engine Plugin Responsibilities:
  MUST:
    ...
    - Translate fragment into Delta Lake-native operation
    - Preserve semantic equivalence
    - Invoke Downstream Datastore through approved Delta Lake interface
...
Capability Declarations:
  ...
  - streaming
  - federation
  - transactions where table transaction semantics are supported
...
Translation Rules:
  ...
  - Delta transaction log semantics MUST be explicit.
  - Snapshot and time travel behavior MUST be preserved where supported.
...
Downstream Invocation Rules:
  ...
  - Log and object storage references MUST preserve tenant and policy constraints.
  - Snapshot selection MUST be deterministic.

Datastore Data Plane Boundary:
  - Delta Lake Downstream Datastore owns its Datastore Data Plane.
  - SDE Data Plane reaches Delta Lake Datastore Data Plane only through approved Engine Plugin.
  - Native execution semantics remain owned by Delta Lake.
  - SDE platform semantics MUST not be silently redefined by Delta Lake native behavior.
...
Result Mapping Rules:
  ...
  - Snapshot metadata, file actions, scan tasks, and rows where applicable MUST map to Result Model.
...
Error Mapping Rules:
  ...
  - Log, checkpoint, object storage, and consistency errors MUST map safely to Error Model.
...
Invariants:
  - Delta Lake Engine Plugin is the downstream execution boundary.
  - Engine Runtime invokes Delta Lake Engine Plugin.
  - Delta Lake Engine Plugin invokes Delta Lake Downstream Datastore.
  - Delta Lake Engine Plugin does not manage datastore lifecycle.
