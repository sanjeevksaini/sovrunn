...
- FEATURE-0013 contract boundaries are clarified by `ADH-2026-016` (joint controlling with ADH-2026-014 and ADH-2026-015; clarification only, no new architecture): `DecisionRecord` uses `metadata.scopeRef` as its sole scope authority (no top-level or parallel scope source); a closed ordered provider-neutral sensitivity vocabulary `PUBLIC < INTERNAL < CONFIDENTIAL < RESTRICTED` with mandatory profile-bound rules; a structural-only security-validation boundary; a structural-trust versus deferred-cryptographic boundary (canonicalization algorithm/profile, digest-covered fields, signature algorithm, and cryptographic services DEFERRED under ADR-F13-002; RFC 8785 illustrative only); exact conformance coverage `F13-CF-01` through `F13-CF-28`; six-scope `AuditEvent` evidence marked SUPERSEDED; and completed `DecisionObject`-to-`DecisionRecord` migration in active normative documents. ADH-2026-016 authorizes no runtime/product/provider/algorithm selection and returned FEATURE-0013 requirements to architecture-remediation status; design/tasks/implementation remain unauthorized.
